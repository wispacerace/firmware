...
#define fault_printf(f, ...)                                                   \
    chprintf((BaseSequentialStream *)(&ms), f "\n", ##__VA_ARGS__)
...
static void _mem_fault(void) {
    ...
    if (SCB->CFSR & SCB_CFSR_DACCVIOL_Msk) {
        ...
        fault_info.decoded_fault_registers.memfault.data_access_violation =
            true;
    }
    if (SCB->CFSR & SCB_CFSR_MMARVALID_Msk) {
        fault_printf("Address: 0x%08x", (uint32_t)SCB->MMFAR);
        fault_info.decoded_fault_registers.memfault
            .data_access_violation_address = (uint32_t)SCB->MMFAR;
    } else {
        fault_printf("Address: unknown");
        fault_info.decoded_fault_registers.memfault
            .data_access_violation_address = 0xffffffff;
    }
    if (SCB->CFSR & SCB_CFSR_IACCVIOL_Msk) {
        fault_printf("Instruction Access Violation");
        fault_info.decoded_fault_registers.memfault
            .instruction_access_violation = true;
    }
}
...
static void _bus_fault(void) {
    ...
    if (SCB->CFSR & SCB_CFSR_PRECISERR_Msk) {
        ...
        fault_info.decoded_fault_registers.busfault.precise_data_bus_error =
            true;
    }
    if (SCB->CFSR & SCB_CFSR_BFARVALID_Msk) {
        fault_printf("Address: 0x%08x", (uint32_t)SCB->BFAR);
        fault_info.decoded_fault_registers.busfault
            .precise_data_bus_error_address = (uint32_t)SCB->BFAR;
    } else {
        fault_printf("Address: unknown");
        fault_info.decoded_fault_registers.busfault
            .precise_data_bus_error_address = 0xffffffff;
    }
    if (SCB->CFSR & SCB_CFSR_IMPRECISERR_Msk) {
        fault_printf("Imprecise data bus error");
        fault_info.decoded_fault_registers.busfault.imprecise_data_bus_error =
            true;
    }
    if (SCB->CFSR & SCB_CFSR_IBUSERR_Msk) {
        fault_printf("Instruction bus error");
        fault_info.decoded_fault_registers.busfault.instruction_bus_error =
            true;
    }
}
...
static void _usage_fault(void) {
    ...
    if (SCB->CFSR & SCB_CFSR_UNALIGNED_Msk) {
        ...
        fault_info.decoded_fault_registers.usagefault.unaligned_memory_access =
            true;
    }
    if (SCB->CFSR & SCB_CFSR_NOCP_Msk) {
        fault_printf("No coprocessor instructions");
        fault_info.decoded_fault_registers.usagefault
            .no_coprocessor_instructions = true;
    }
    ...
    if (SCB->CFSR & SCB_CFSR_UNDEFINSTR_Msk) {
        ...
        fault_info.decoded_fault_registers.usagefault.undefined_instruction =
            true;
    }
}
...
#ifndef FAULT_NO_PRINT
    msObjectInit(&ms, (uint8_t *)fault_info.decoded_info_string,
...
static void _save_fault_info(void) {
    ...
    if (ch.rlist.current) {
        fault_printf("Thread: 0x%08x, %s", ch.rlist.current,
                     ch.rlist.current->name);

        fault_info.decoded_fault_registers.general.current_thread_address =
            (uint32_t)ch.rlist.current;
        fault_info.decoded_fault_registers.general.current_thread_name =
            ch.rlist.current->name;
    } else {
    ...
    if (!(SCB->HFSR & (SCB_HFSR_VECTTBL_Msk | SCB_HFSR_FORCED_Msk))) {
