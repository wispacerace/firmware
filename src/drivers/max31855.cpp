...
quantity<absolute<celsius::temperature>, float>
demangle_temp_thermocouple(uint16_t thermocouple_temp) {
    // thermocouple_temp is a 14-bit 2's complement signed integer, packed into
    // a uint16_t. to get a int16_t out of this, we'll shift it left two bits to
    // get the sign bit where it should be, and then divide by 1<<2 = 4 to
    // restore the magnitude.
    auto signed_temp =
        bit_cast<int16_t>((uint16_t)(thermocouple_temp << 2)) / 4;

    // we'll now turn it into a float and wrap it with proper units;
    // the raw thermocouple temp is given in units of 0.25°C.
    return static_cast<float>(signed_temp) * 0.25f *
           absolute<celsius::temperature>();
}

quantity<absolute<celsius::temperature>, float>
demangle_temp_internal(uint16_t internal_temp) {
    // internal_temp is a 12-bit 2's complement signed integer, packed into a
    // uint16_t. to get a int16_t out of this, we'll shift it left four bits to
    // get the sign bit where it should be, and then divide by 1<<4 = 16 to
    // restore the magnitude.
    ...
    return static_cast<float>(signed_temp) * 0.0625f *
           absolute<celsius::temperature>();
}
...
quantity<absolute<celsius::temperature>, float>
Max31855Reading::internal_temp() const {
...
quantity<absolute<celsius::temperature>, float>
Max31855Reading::thermocouple_temp() const {
#ifdef MAX31855_DEBUG
