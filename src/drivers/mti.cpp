...
void MtiIMU::handle_data(IMUSyncPipe *sync) {
    ...
    if (status.notification_message_size > 0) {
        ...
        this->read_data(XBUS_NOTIFICATION_PIPE, status.notification_message_size, sync);
    } else if (status.measurement_message_size > 0) {
        // printf("processing measurements\n");
        this->read_data(XBUS_MEASUREMENT_PIPE,  status.measurement_message_size, sync);
    }
}
...
void MtiIMU::read_data(uint8_t pipe, uint16_t length, IMUSyncPipe *sync) {
    ...
    spiReceive(&this->m_spi_driver, length, this->m_message_buffer); // and drain (length) bytes out of it
    spiUnselect(&this->m_spi_driver);
    ...
    while (idx < length) {
        ...
        if (msg != nullptr) {
            auto parsed_length = xbus_buf2msg(msg, this->m_message_buffer + idx, length - idx);
            if (parsed_length != -1) {
                idx += parsed_length;
                // printf("!\n");

                if (msg->id == XMID_MtData2) {
                    chMBPostTimeout(&sync->datamsg_mbox, (msg_t)msg, TIME_IMMEDIATE);
                } else {
                    chMBPostTimeout(&sync->auxmsg_mbox, (msg_t)msg, TIME_IMMEDIATE);
                }
            } else {
...
void MtiIMU::set_output_config(tcb::span<mti::OutputConfigPair> cfg) {
    ...
    XbusMessage msg = {.mid = XMID_SetOutputConfig, .length = (uint16_t) payload_length, .data = data};
    this->send_message(&msg);
}
...
void MtiIMU::go_to_state(mti::DeviceState state) {
    XbusMessage msg = {};
    switch (state) {
        case mti::DeviceState::Configuration: msg.mid = XMID_GotoConfig; break;
        case mti::DeviceState::Measurement: msg.mid = XMID_GotoMeasurement; break;
    }
...
mtssp_status MtiIMU::read_status() {
    ...
    mtssp_status status = {
        .notification_message_size = (uint16_t)(status_data[0] | (status_data[1] << 8)),
        .measurement_message_size  = (uint16_t)(status_data[2] | (status_data[3] << 8)),
    };
...
XbusParsedMessage* MtiIMU::transact_message(XbusMessage *tx, IMUSyncPipe *sync) {
    this->send_message(tx);

    XbusParsedMessage *rx;
    auto res = chMBFetchTimeout(&sync->auxmsg_mbox, (msg_t*)&rx, TIME_MS2I(500));
    return (res == MSG_OK) ? rx : nullptr;
}

size_t mti::format_output_config(uint8_t *buf, tcb::span<OutputConfigPair> cfg) {
        size_t idx = 0;
